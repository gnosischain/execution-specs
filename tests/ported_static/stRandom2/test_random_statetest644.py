...
def test_random_statetest644(
    ...
    # Tx calls the ecrecover precompile; gas_limit is the fork's
    # intrinsic cost (which includes any 7623/7976 floor) plus the
    # precompile's fixed cost. Works on every fork.
    gas_limit = (
        fork.transaction_intrinsic_cost_calculator()(calldata=data)
        + fork.gas_costs().PRECOMPILE_ECRECOVER
    )
    tx = Transaction(
        sender=sender,
