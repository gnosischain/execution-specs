...
def _exact_size_transactions_impl(
    ...
    if remaining_bytes > 0 and remaining_gas > 25_000:
        ...
        if target_gas <= remaining_gas:
            ...
            if test_size == block_size_limit:
                ...
                # Increased search range for Gnosis for large adjustments
    ...
    # Allow larger tolerance for Gnosis - different transaction types have
    # varying RLP overhead. The block extra_data field will be used to adjust
    # to the exact size in the test.
...
def test_block_at_rlp_size_limit_boundary(
    ...
    assert abs(block_rlp_size - block_size_limit) <= tolerance, (
        f"Block RLP size {block_rlp_size} does not match limit {block_size_limit} "  # noqa: E501
        f"within tolerance {tolerance}, difference: {block_rlp_size - block_size_limit} "  # noqa: E501
        "bytes"
    )
