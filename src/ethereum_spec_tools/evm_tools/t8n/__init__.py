...
class T8N(Load):
    ...
    def _run_blockchain_test(self, block_env: Any, block_output: Any) -> None:
        ...
        if (
            ...
            and self.state_reward != -1
        ):
            # ``-1`` is the sentinel for "skip generic block rewards"
            # (testing-side ``TransitionToolData.__post_init__`` sets this
            # for genesis blocks). Forks with ``process_block_rewards`` use
            # their chain-specific reward mechanism instead.
